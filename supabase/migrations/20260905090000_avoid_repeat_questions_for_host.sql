-- Ismétlődés elleni védelem: X napig ne jöhessen szembe ugyanaz a kérdés a
-- szobát INDÍTÓ profilnak, még akkor sem, ha közben egy másik szobát nyit.
--
-- KORÁBBAN a kérdésismétlés-elkerülés csak EGYETLEN szoba élettartamára
-- vonatkozott (`not exists (... where rq.room_id = p_room ...)`) – ha valaki
-- ugyanazzal a fiókkal másnap új szobát nyitott, elölről kaphatta ugyanazokat
-- a kérdéseket. Ez most kiegészül egy 30 napos, HOST-PROFILHOZ kötött
-- ablakkal: a `rooms.host_id` alapján megnézzük, mit kapott ez a profil bármely
-- szobájában az elmúlt 30 napban, és azt előnyben kerüljük – de csak
-- PREFERENCIAKÉNT, nem kőbe vésett szabályként: ha egy kategóriában vagy az
-- egész bankban már csak "nemrég látott" kérdés maradna, inkább azt adjuk ki,
-- mint hogy feleslegesen véget érjen a játék. Ezt a régi, funkció előtti
-- viselkedésre visszaeső fallback-lépések biztosítják.
--
-- Miért a HOST-hoz kötve, nem az összes résztvevőhöz? A host az, aki a
-- legtöbb szobát nyitja/játssza a saját fiókjával, tehát ő szembesülne
-- leghamarabb az ismétlődéssel; az `host_id`-n már van index
-- (`rooms_host_idx`), így ez a szűrés olcsón kérhető le. Ha valaki csak
-- csatlakozóként játszik sok szobában, rá ez a védelem nem vonatkozik – ez a
-- kérés (a szobát KÉSZÍTŐ profil) szerint szándékos.
--
...
-- A host-történet lekérdezése (host_id szerint, dátum-tartománnyal szűrve)
-- gyakori lesz minden egyes tick-nél – ez az index ezt gyorsítja.
create index if not exists room_questions_started_idx
  on public.room_questions (started_at);
...
begin
  ...
  if v_rq.id is null
    ...
    if v_rq.id is not null and ((v_next_ord - 1) % v_room.questions_per_category) <> 0 then
      ...
      -- de a host-történetet még próbáljuk tartani, amíg lehet.
